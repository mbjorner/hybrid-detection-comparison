...
TicrOut = DataFrame(CSV.File(ARGS[1]))
#Expected = DataFrame(CSV.File(ARGS[1])) # sometimes there are problems with the expected values, such as in intersecting cycles
# this means that we will not produce an expected table for quartets, we already find presence of hybrids from another form, so this is removed.
MSCOut = DataFrame(CSV.File(ARGS[2]))
...
#create new matrix of size size(expectedVals, 1) == number of quartets
hybridExpected = Vector{Bool}(undef, setsOfQuartets)
...
hybridMSC = Vector{Bool}(undef, setsOfQuartets)
...
CSV.write(string(outfile), TicrOut)
