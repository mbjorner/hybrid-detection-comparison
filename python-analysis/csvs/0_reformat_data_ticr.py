...
nets = [ "n4h1_0.1", "n4h1_0.2", "n4h1_0.3", "n4h1_0.4", "n4h1_0.5",
         "n4h1_0", "n4h1_introg", "n5h2", "n8h3", "n10h2",
         "n10orange", "n10red", "n15blue", "n15h3", "n15orange",
         "n15red", "n25h5"]
...
for net in nets:
    for gtre in gtres:
        ...
        for ngen in ngens:
            ...
            if (net == "n4h1_0.5") and (ngen == 30):
                repls.remove(26)
            for repl in repls:
                xdf = df[(df["network_name"] == net + ".net") &
                         (df["gene_trees"] == ngen) &
                         (df["trial_num"] == repl)]
                
                if xdf.shape[0] != 1:
                    print(xdf)
                    sys.exit("Double check %s %s %d %d" % (net, gtre, ngen, repl))
                else:
